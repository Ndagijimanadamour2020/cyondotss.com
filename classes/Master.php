...
Class Master extends DBConnection {
	...
	function save_article(){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = " New Announcement has been successfully added!";
			else
				$resp['msg'] = " Announcement's Details has been updated successfully!";
			...
			if(!$save_content){
				if(!empty($id)){
					$resp['msg'] = " Announcement's Details has been updated successfully but content has failed!";
				}else{
					$resp['status'] = 'failed';
					$resp['msg'] = " Announcement has failed to add due to an error occurred while saving the content file!";
			...
			$resp['status'] = 'failed';
			if(empty($id))
				$resp['msg'] = " New Announcement has failed to save!";
			else
				$resp['msg'] = " Announcement's Details has failed to update!";
			$resp['error'] = $this->conn->error;
	...
	function delete_article(){
		...
		if($update){
			$resp['status'] = 'success';
			$resp['msg'] = ' Announcement has been deleted successfully!';
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = ' Announcement has failed to delete!';
			$resp['error'] = $this->conn->error;
		}
